...
class ApiClient {
  ...
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<{ data: T }> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Create a properly serializable error object
        const apiError: ApiError = {
          status: error.status || response.status,
          message: error.message || response.statusText || `HTTP ${response.status}`,
      ...
      // If error is already an ApiError, throw as is
   ...
   * URL pattern: /api/{user_id}/books
   * Backend returns BookListResponse with { data: Book[] }
   * API client's get() extracts response.data, so we get { data: Book[] } directly
   */
  async fetchBooks(): Promise<Task[]> {
    try {
      const response = await this.get<{ data: Task[] }>(`/api/${this.userId}/books`);
      // Backend returns { data: Book[] }, extract the inner data array
      ...
      // Ensure error is properly formatted before re-throwing
      if (error && typeof error === 'object' && 'status' in error && 'message' in error) {
        // Already a proper ApiError, re-throw as is
        throw error;
      }
      // Convert to ApiError format
      const apiError: ApiError = {
        status: error?.status || 0,
        message: error?.message || error?.detail || String(error) || 'Failed to fetch books',
        details: error,
      };
      throw apiError;
    }
  ...
  /**
   * Create a new book
   * URL pattern: POST /api/{user_id}/books
   * Backend returns BookResponse directly (wrapped in { data: ... } by request method)
   */
  async createBook(input: CreateTaskInput): Promise<Task> {
    const response = await this.post<{ data: Task } | Task>(`/api/${this.userId}/books`, input);
  ...
  /**
   * Update a book (title/description update)
   * URL pattern: PUT /api/{user_id}/books/{book_id}
   * Backend returns BookResponse directly (wrapped in { data: ... } by request method)
   */
  async updateBook(bookId: string, updates: UpdateTaskInput): Promise<Task> {
    // PUT endpoint expects all book fields
    const { title, description, due_date, due_date_end, priority, category } = updates;
    if (!title) {
      throw new Error('Title is required for book update');
    }
    ...
    const response = await this.put<{ data: Task } | Task>(`/api/${this.userId}/books/${bookId}`, updateData);
    // Handle both wrapped and direct responses
    if (response && typeof response === 'object' && 'data' in response) {
      return (response as { data: Task }).data;
    }
  ...
  /**
   * Patch a book's completion status
   * URL pattern: PATCH /api/{user_id}/books/{book_id}
   * Backend returns BookResponse directly (wrapped in { data: ... } by request method)
   */
  async patchBookStatus(bookId: string, completed: boolean): Promise<Task> {
    // PATCH endpoint only accepts { completed: bool }
    const response = await this.patch<{ data: Task } | Task>(`/api/${this.userId}/books/${bookId}`, { completed });
    // Handle both wrapped and direct responses
    if (response && typeof response === 'object' && 'data' in response) {
      return (response as { data: Task }).data;
    }
  ...
  /**
   * Delete a book
   * URL pattern: DELETE /api/{user_id}/books/{book_id}
   */
  async deleteBook(bookId: string): Promise<void> {
    await this.delete(`/api/${this.userId}/books/${bookId}`);
  }
  ...
  async updateTask(taskId: string, updates: UpdateTaskInput): Promise<Task> {
    return this.updateBook(taskId, updates);
  }

  async patchTaskStatus(taskId: string, completed: boolean): Promise<Task> {
    return this.patchBookStatus(taskId, completed);
  }

  async deleteTask(taskId: string): Promise<void> {
    return this.deleteBook(taskId);
  }
}
