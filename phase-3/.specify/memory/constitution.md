...
Added sections: Principle XI (AI-Powered Conversational Interface), Phase III technology stack additions
...
-->
# Phase III: Full-Stack Web Evolution with AI Chatbot - Project Constitution
...
### VII. API Design Standards (RESTful with CRUD + PATCH + Chat)
...
- **Operations**:
  ...
  - `POST /api/{user_id}/chat` - (Phase III) Send message and receive AI response via stateless chat endpoint
- **Response Format**: Standardized JSON using Pydantic schemas with consistent error handling
- **Chat Endpoint**: (Phase III) Must be stateless, fetch conversation history from database, invoke MCP tools via AI agent, store messages in database

**Rationale**: RESTful conventions provide predictable, self-documenting APIs that frontend developers can easily consume. Stateless chat endpoint enables horizontal scaling and resilience.
...
### XI. AI-Powered Conversational Interface (NON-NEGOTIABLE for Phase III)
The application MUST support natural language task management through an AI-powered conversational interface:
...
**Rationale**: Stateless chat architecture enables horizontal scaling and resilience. MCP tools provide standardized interface for AI to interact with application. Database-backed conversations ensure persistence across server restarts.
...
- **AI Framework**: OpenAI Agents SDK (Phase III) - for AI agent logic and natural language processing
- **MCP Server**: Official MCP SDK (Phase III) - for exposing task operations as MCP tools
...
- **Chat UI**: OpenAI ChatKit (Phase III) - for conversational interface
...
All environment variables MUST be documented in `.env.example` and stored securely:

**Backend (Required)**:
- `BETTER_AUTH_SECRET` - Shared secret for JWT signing/verification (Phase II)
- `DATABASE_URL` - Neon PostgreSQL connection string (Phase II)
- `OPENAI_API_KEY` - OpenAI API key for Agents SDK (Phase III)

**Frontend (Required)**:
- `BETTER_AUTH_SECRET` - Shared secret for JWT signing (Phase II)
- `BETTER_AUTH_URL` - Frontend URL for Better Auth (Phase II)
- `NEXT_PUBLIC_API_URL` - Backend API URL (Phase II)
- `NEXT_PUBLIC_OPENAI_DOMAIN_KEY` - OpenAI ChatKit domain key (Phase III)

**Rationale**: Environment variables provide secure configuration management and enable different settings for development, staging, and production environments.
...
│   │   │   └── chat/         # Phase III: ChatKit UI components
...
│   └── pyproject.toml
├── specs/
│   ├── overview.md           # Phase III: Project overview
